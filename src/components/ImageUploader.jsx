...
const ImageUploader = ({ type, onUpload, maxFiles, required }) => {
  ...
  const handleDeletePicture = (index) => {
    ...
    onUpload(newFilenames.map((_, i) => fileInputRef.current.files[i]));
  };
    ...
    <section className="container w-full mx-auto items-center">
      <div>
        ...
        <div className="px-4 py-6">
          <div
            id="image-preview"
            className={`max-w-sm p-6 mb-4 relative ${previews.length ? '' : 'bg-gray-100 border-dashed border-2 border-gray-400'} rounded-lg items-center mx-auto text-center cursor-pointer`}
          >
            {previews.length ? (
              <>
                {previews.map((preview, index) => (
                  <div key={index} className="relative mb-4">
                    <button type="button" onClick={() => handleDeletePicture(index)} className="absolute top-0 right-0 p-1 bg-red-500 rounded-full text-white hover:bg-red-600 focus:outline-none focus:bg-red-600 transition duration-150 ease-in-out">
            ...
            <span id="filename" className="text-gray-500 bg-gray-200 z-50 block max-w-xs overflow-hidden">{filenames.join(', ')}</span>
          </div>
        </div>
      </div>
    </section>
